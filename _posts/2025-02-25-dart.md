...
- `.reduce(Callback)`
  ...
  > `myList.reduce((val, elem) => val + elem);`  // 6
- `.fold(InitialValue, Callback)`
  - If list it empty, return `InitialValue`. 
  > `myList.fold(0, (val, elem) => val + elem);`  // 6
...
Return 1 value
- `.remove(Key)` // pop `Key`
    - If Map[`Key`] is empty, return `null`. 
